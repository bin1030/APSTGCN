...
class STLayer(nn.Module):
    def __init__(self, n_residuals: int, n_dilations: int, kernel_size: int, dilation: int,
                 n_skip: int, edge_dim: int, dropout: float): #n_dilations=40
        ...
        self.shape_conv = nn.Conv2d(n_residuals, n_dilations, kernel_size=(1, kernel_size), dilation=dilation)
        # Added attention mechanism
        self.attention = Attention(n_dilations)  
    ...
    def forward(self, x: Tensor, skip: Tensor, supports: Tensor):
        ...
        g = self.shape_conv(g)
        g = torch.relu(g)
        gconv_output = self.gconv(g, supports)
        x = gated_output+gconv_output
