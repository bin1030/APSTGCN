...
class GraphLearner(nn.Module):
    def __init__(self, supports: Tensor, n_hist: int, n_in: int, node_dim: int, dropout: float, learn_macro: bool,
                 learn_micro: bool):
        ...
        if learn_micro:
            ...
            self.coefficient = nn.Parameter(torch.tensor(1.0), requires_grad=True)
    ...
    def forward(self, inputs: Tensor = None) -> Tensor:
        ...
        if hasattr(self, 'mi_learner'):
            # supports = self.coefficient * self.adaptive.unsqueeze(1) + (1 - self.coefficient) * self.mi_learner(inputs)
            supports = self.adaptive.unsqueeze(1) + self.coefficient * self.mi_learner(inputs)
